...
class Portfolio extends Component {
    ...
    portfolioTitleHandler = () => {
        ...
        if (document.body.scrollTop > portfolio || document.documentElement.scrollTop > portfolio ){
            document.getElementById("porthead").className = "porthead";
            document.getElementById("portfoliounderline").className = "portfoliounderline";
        }
    }
    workFadeHandler = () => {
        ...
        return (
            ...
            <div id="portfolio" className="portfolio">
                <div className="portfoliotitle">
                    <h2 id="porthead" className="porthead-clear">Portfolio.</h2>
                    <div id="portfoliounderline" className="underline-clear"/>
                </div>
                <div id="slides-animation" className="slides-clear">
                    {workList.map((el,i)=>(
                        <Work key={i} value={el.value} image={el.image} alt={el.alt} title={el.title} tech={el.tech}  click={this.props.click} />
                    ))}
                </div>
            </div>
        )
